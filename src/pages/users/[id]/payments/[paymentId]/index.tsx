...
const Show = ({ user }) => {
    ...
    try {
      ...
      const res = await fetch("/api/acceptRefundRequest", {
        ...
        body: JSON.stringify({
          ...
          reasonText: payment.refund.reasonText,
          detailText: payment.refund.detailText,
  ...
  const refundForm = useMemo(() => {
    ...
    if (payment.refund && payment.refund.rejected) return <p>返金申請が拒否されました。管理者により審査が行われます。</p>;
    if (payment.refund && payment.buyer === user.uid && moment(payment.createdAt.toDate()).isBefore(moment().subtract(3, 'days'))) return (
      <>
        <p>申請から3日間が経過しました。以下のボタンから返金を確定できます。</p>
        <Button type='button' color='success' onClick={acceptRefund}>返金を確定させる</Button>
      </>
    )
    if (!payment.refund && payment.buyer === user.uid) return (
      <Link href={`/users/${user.uid}/payments/${payment.id}/refund`}>
        <Button color="danger">返金申請する</Button>
      </Link>
    );
    if (payment.refund && payment.buyer === user.uid) return (
      <Button type='button' color='secondary'>返金申請の手続き中です</Button>
    )
    if (payment.refund && payment.seller === user.uid) return (
      <>
        <p>返金が申請されました。確認の上、受理の可否を選択してください。<br/>拒否した場合には、管理者による審査が行われます。</p>
        <p>{`理由: ${payment.refund.reasonText}`}</p>
        <p>{`詳細: ${payment.refund.detailText}`}</p>
        <Row form>
          <Col>
            <Button type='button' color='success' onClick={acceptRefund} style={{ marginRight: '1em' }}>受理</Button>
            <Button type='button' color='danger' onClick={rejectRefund}>拒否</Button>
          </Col>
        </Row>
      </>
    )
  ...
  return (
    <>
      ...
      <p>{`購入日時: ${moment(payment.createdAt.toDate()).format(
        "YYYY年 M月D日 H:mm"
      )}`}</p>
