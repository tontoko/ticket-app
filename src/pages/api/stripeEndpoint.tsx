...
const Webhock: NextApiHandler = async (req, res) => {
  ...
  try {
    ...
    webhockEvent = stripe.webhooks.constructEvent(buf.toString(), sig, endpointSecret)
  } catch (err) {
    // invalid signature
  ...
  try {
    await firestore.runTransaction(async (transaction) => {
      ...
      if (stock === 0 || stock - sold < 1) {
        throw new NoBuyableError('在庫がありませんでした。返金手続きが行われました。')
      }
      if (!categoryResult.public) {
        throw new NoBuyableError('購入したチケットは非公開になりました。返金手続きが行われました。')
      }
      // 売り上げを一つ増やす
      transaction.set(
        categoryRef,
